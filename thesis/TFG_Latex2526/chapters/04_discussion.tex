...
\vspace{0.4em}
\noindent
When processing the 40 cases, the pipeline remained stable for both the ASOCA and MACS-18 cohorts, achieving an Execution Success Rate of 100\%. The richer anatomical fidelity of MACS-18 improves the topology recovery in Block~1. As shown in \Cref{tab:endpoint-summary}, the mean detected endpoints increased from 17.1 to 27.7 per case, and the Centerline Extraction Success Rate improved from 83.19\% to 95.62\% (\Cref{tab:ostium-centerline-comparison}). These gains suggest that MACS-18 better preserves distal continuity and lumen details, allowing the pathfinding algorithm to traverse segments that were previously disconnected or lost (\Cref{fig:centerlines-surfaces-comparison}).
...
Rather than merely summarizing the numerical results, it is important to interpret what they mean for the dashboard's design. The most positive signals relate to the system's perceived clinical utility and visual integration. Strong ratings on the intent for frequent use (Question~1) and the integration of 3D meshes with 2D plots (Question~5) indicate that the core concept resonates well with clinical needs. Conversely, responses regarding diagnostic confidence (Question~9) and the need for technical support (Question~4) were more mixed. Referencing the full questionnaire provided in \AppCref{app:sus-questionnaire}, these varied answers suggest that while the visual layout is highly appreciated, the tool requires further refinement before clinicians can feel completely autonomous and confident using it without prior training; specific interface improvements proposed by participants are outlined in \Cref{subsec:future-clinical-proposals}.

Beyond the scores, the session produced concrete proposals for future interface iterations (\Cref{sec:discussion-future}). Participants' overall impressions were positive, affirming practical utility within the clinical ecosystem at Hospital de la Santa Creu i Sant Pau.
...
\begin{figure}[H]
  ...
  \caption{Area overestimation near a coronary bifurcation. The 3D model maps \%AS (green colormap); the inset marks the maximum stenosis point (purple diamond) and automatic proximal/distal references (green circles). Bar plots show cross-sectional area (blue) and \%AS (orange), including the artificial spike after the bifurcation.}
